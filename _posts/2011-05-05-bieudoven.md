...
accordion:
  ...
  - title: General theory
    content: <p> In general, the same proof can be applied to prove the more general case with n sets \(A_i\) for \(i \in \{1,2, \cdots ,n\} \), which states that \[\mid A \cup B \mid = \mid \cup_{i}A_i \mid - \mid \cap_{i,j}A_{i,j} \mid + \mid  \cap_{i,j,k}A_{i,j,k}\mid - \cdots \], and so on.
---
<head>
  <meta charset="utf-8">
